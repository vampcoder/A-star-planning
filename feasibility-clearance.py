...
class pixel(object):
    def __init__(self, penalty, pointx, pointy, depth): # parent is that pixel from which this current pixel is generated
...
def fill_clearance(arr,cmax,  final_contours): # sx, sy :- source coordinates  dx, dy :- destination coordinates
    ...
    for cnt in final_contours:
        for pts in cnt:
            q.put(pixel(0, pts[0, 1], pts[0, 0], 0))
    cnt = 0
    cntq = 0
    while not q.empty():
        p = q.get()
            ...
            if check_boundaries(ex, ey, nx, ny) == True:
                if arr.item(nx, ny, 0) == 0 and arr.item(nx, ny, 1) == 0 and arr.item(nx, ny, 2) == 0:
                    if min_cost[nx][ny] > penalty(x, y, nx, ny, pen):
                        q.put(pixel(penalty(x,y,nx,ny,pen), nx, ny, p.depth+1))
    return min_cost
       ...
       #         print'aaa'
        ...
        for i in xrange(x):
            for j in xrange(y):
               ...
               # if j>=240 and j<=250 and i>=110 and i<=120:
                #    print j,i,min_cost[j][i]
